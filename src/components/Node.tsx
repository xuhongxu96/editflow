...
export type OnNodeHandleMouseDownEventListener = (
  e: React.MouseEvent,
  id: string,
  direction: HandleDirection
) => void;
export type OnNodePortMouseEventListener = (
  e: React.MouseEvent,
  id: string,
  port: Flow.Port,
  io: 'input' | 'output',
...
export type NodePortEnableCallback = (
  id: string,
  port: Flow.Port,
  io: 'input' | 'output',
  portIndex: number
  ...
  return (
    ...
    >
      ...
      <PortBox
        input={input}
        output={output}
        onPortMouseDown={(e, port, io, index) =>
          onPortMouseDown && onPortMouseDown(e, id, port, io, index)
        }
        onPortMouseUp={(e, port, io, index) =>
          onPortMouseUp && onPortMouseUp(e, id, port, io, index)
        }
        onPortMouseEnter={(e, port, io, index) =>
          onPortMouseEnter && onPortMouseEnter(e, id, port, io, index)
        }
        onPortMouseLeave={(e, port, io, index) =>
          onPortMouseLeave && onPortMouseLeave(e, id, port, io, index)
        }
        enableCallback={(port, io, index) =>
          enabledPortType ? enabledPortType(id, port, io, index) : true
        }
      />
